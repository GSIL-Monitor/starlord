...
class Base extends CI_Controller
{
    ...
    public function __construct()
    {
        ...
        $logContent = $logDate . ' | ' . 'request_in' . ' | ' . self::$traceId . ' | ' . $user['user_id'] . ' | ' . json_encode($input);
        error_log($logContent . "\n", 3, Config::BIZ_ACCESS_LOG_PATH);
    ...
    protected function _returnSuccess($data = array())
    {
        //request_error_log
        $logDate = date("Y-m-d H:i:s", time());
        $logContent = $logDate . ' | ' . 'request_out' . ' | ' . Base::$traceId . ' | ' . 'errno=0';
        error_log($logContent . "\n", 3, Config::BIZ_ACCESS_LOG_PATH);
