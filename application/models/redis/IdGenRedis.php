...
EOF;
        $this->sHash = $this->script('load',$sScript);
    }


    public function gen($sKey = 'default', $iPartition = 0)
    {
        $aResult = $this->evalsha($this->sHash, array($sKey, $iPartition), 2);

        return (($aResult[0] * 1000 + $aResult[1]/1000)<<(12+10))+(1<<10)+$aResult[3];
    }
}
